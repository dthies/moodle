    ...
    TEMPLATES = {
        FORM: '' +
            '<form class="atto_form">' +
                ...
                '<div describedby="{{elementid}}_cursorinfo" class="well well-small fullwidth {{CSS.EQUATION_PREVIEW}}" style="position: relative; left: 0px; top: 0px" id="{{elementid}}_{{CSS.EQUATION_PREVIEW}}"></div>' +
        ...
        LIBRARY: '' +
            '<div class="{{CSS.LIBRARY}}">' +
                ...
                '</div>' +
            '</div>',
        PREVIEW: '<div describedby="{{elementid}}_cursorinfo" class="well well-small fullwidth {{CSS.EQUATION_PREVIEW}}" style="position: absolute; left: 0px; top: 0px" id="{{elementid}}_{{CSS.EQUATION_PREVIEW}}" alt="{{content}}"></div>'
    };
...
Y.namespace('M.atto_equation').Button = Y.Base.create('button', Y.M.editor_atto.EditorPlugin, [], {
    ...
    _displayDialogue: function() {
        ...
        this._previewNode = this._content.one(SELECTORS.EQUATION_PREVIEW);
    ...
    _updatePreview: function(e) {
        ...
        // If equation has not changed keep the old preview.
        if (this._previewDisplayed===equation) {
            return;
        }
        if (typeof this._previewList[equation] === 'Object') {
            this.previewDisplayed = equation;
            this._previewNode.appendChild(this._previewList[equation]);
            return;
        }
     ...
     * @private
     */
    _loadPreview: function(id, preview, equation) {
        if (preview.status === 200) {
            var template = Y.Handlebars.compile(TEMPLATES.PREVIEW);
            var node = this._previewNode.appendChild(Y.Node.create(template({
                elementid: this.get('host').get('elementid'),
                content: preview.responseText,
                CSS: CSS
            })));
            node.setHTML(preview.responseText),
            this._previewList[equation] = node;
            ...
            this._previewDisplayed = equation;
        }
    ...
    _getDialogueContent: function() {
        ...
        this._content.one(SELECTORS.EQUATION_TEXT).on('valuechange', this._throttle(this._updatePreview,50), this);
        this._content.one(SELECTORS.EQUATION_TEXT).on('mouseup', this._throttle(this._updatePreview, 50), this);
        this._content.one(SELECTORS.EQUATION_TEXT).on('keyup', this._throttle(this._updatePreview, 50), this);
