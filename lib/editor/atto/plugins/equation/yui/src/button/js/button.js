...
var COMPONENTNAME = 'atto_equation',
    ...
    CSS = {
        ...
        EQUATION_LOADED: 'atto_equation_loaded',
    ...
    SELECTORS = {
        ...
        EQUATION_LOADED: '.' + CSS.EQUATION_LOADED,
...
Y.namespace('M.atto_equation').Button = Y.Base.create('button', Y.M.editor_atto.EditorPlugin, [], {
    ...
    _updatePreview: function(e) {
        ...
        if (equation.substring(0,currentPos + 1).match(/\\[a-zA-Z]+$/)) {
            currentPos += equation.substring(currentPos).match(/[a-zA-Z]*/)[0].length;
        }
        // Next make sure cursor is not preceded by an unescaped backslash.
        if (equation.substring(0,currentPos).match(/\\+$/)) {
            currentPos -= equation.substring(0,currentPos).match(/\\+$/)[0].length % 2;
        }
...
console.log(this._previewList[equation]);
            this._previewDisplayed = equation;
            this._previewNode.appendChild(this._previewList[equation]);
            this._previewNode.all(SELECTORS.EQUATION_LOADED).setStyle('display','none');
            this._previewNode.all(SELECTORS.EQUATION_LOADED).pop().setStyle('display','inline');
            return;
        }
        ...
        if (preview.status === 200) {
            ...
            if (node.one('img')) {
                Y.io(node.one('img').getAttribute('src'), {
                    ...
                    on: {
                        success: function () {
                            node.addClass(CSS.EQUATION_LOADED);
                            this._previewNode.all(SELECTORS.EQUATION_LOADED).setStyle('display','none');
                            this._previewNode.all(SELECTORS.EQUATION_LOADED).pop().setStyle('display','inline');
                        }
                    }
