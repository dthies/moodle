...
EditorSelection.prototype = {
    ...
    setupSelectionWatchers: function() {
        ...
        this.editor.on(['keyup', 'focus', 'dragstart', 'dragend'], function(e) {
                Y.soon(Y.bind(this._hasSelectionChanged, this, e));
            }, this);

        this.editor.on('drop', function(e) {
                // Make sure editor has focus so that the selection can be saved.
                this.editor.focus();
                Y.soon(Y.bind(function(e) {
                    var selection = this.getSelection(),
                        endContainer = selection[0].anchorNode,
                        endOffset = selection[0].anchorOffset,
                        startContainer = endContainer,
                        startOffset = 0,
                        container;
                    // IE does not have the complete dataTransfer so we return to avoid error.
                    if (!e._event.dataTransfer.types[0]) {
                        return;
                    }
                    // Otherwise we load the transfer data into and DOM node to look at.
                    container = Y.Node.create('<div>'
                             + (e._event.dataTransfer.getData('text/html')
                                 || e._event.dataTransfer.getData('text/plain'))
                             + '</div>');
                    if (!container.getHTML()) {
                    }

                    // Check that there is HTML or plain text inserted, but not selected. This needs to be fixed only in Firefox.
                    if (!selection[0].collapsed || !container.getDOMNode().firstChild) {
                        this._hasSelectionChanged(e);
                        return;
                    }

                    if (container.one('*')) {
                        // Find the first Type 2 node after the cursor for referrence.
                        var anchor = (endContainer.nodeName !== '#text' && endContainer.childNodes.item(endOffset)) || endContainer;
                        while (!anchor.nextSibling || anchor.nextSibling.nodeName === '#text') {
                            anchor = anchor.nextSibling || anchor.parentNode;
                        }
                        anchor = anchor.nextSibling;

                        // Count the number of inserted Type 2 nodes.
                        var count = container.all('*').size();

                        // Now backwards the number of inserted Type 2 nodes from the referrence node or end of editor.
                        startContainer = this.editor.all('*').item(this.editor.all('*').size() - count).getDOMNode();
                        this.editor.all('*').each(function(node, index, list) {
                            if (node.compareTo(anchor)) {
                                startContainer = list.item(index - count).getDOMNode();
                            }
                        });

                        // If the first node inserted is text, it will merge with any preceding text and be the start container.
                        if (container.getDOMNode().firstChild.nodeName === '#text') {
                            startContainer = startContainer.previousSibling;
                            startOffset = startContainer.length - container.getDOMNode().firstChild.length;
                        }
                    } else {
                        // Only plain text was moved into a text node so we only adjust starting point in the node.
                        startOffset = endOffset - container.getDOMNode().firstChild.length;
                    }
                    // Change the selection and trigger appropriate actions for saving it.
                    selection[0].setStart(startContainer, startOffset);
