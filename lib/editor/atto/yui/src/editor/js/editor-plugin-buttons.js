    ...
    addButton: function(config) {
        ...
        this._buttonHandlers.push(
            this.toolbar.delegate('click', function(e) {
                var highlight = this.get('host').selectionFilterMatches(config.tags, e.selectedNodes, tagMatchRequiresAll);

                this.get('host')._formatQueue.forEach(function(op) {
                    highlight = ((config.tags === op.tag) !== highlight);
                    highlight = ((config.buttonName !== op.untag) && highlight);
                });
                if (highlight) {
                    ...
                    this.unHighlightButtons(config.buttonName);
                        ...
                        if (config.untag) {
                            this.get('host').toolbar.all('.atto_' + config.untag + '_button_' + config.untag).removeClass('highlight');
                            console.log('atto_' + config.untag + '_button_' + config.untag);
                        }
            }, '.' + buttonClass, this)
        );
            ...
            this._buttonHandlers.push(
                host.on(['atto:selectionchanged', 'change'], function(e) {
                    ...
                    this._highlightQueue[config.buttonName] = Y.soon(Y.bind(function(e) {
                        var highlight = host.selectionFilterMatches(config.tags, e.selectedNodes, tagMatchRequiresAll);
                        if (highlight) {
                            this.highlightButtons(config.buttonName);
                        } else {
                            this.unHighlightButtons(config.buttonName);
    ...
    _normalizeCallback: function(config, inheritFrom) {
        ...
        if (config.inlineFormat && typeof config._inlineCallback === 'function') {
            config._callback = function(e, args) {
                this.get('host').applyFormat(e, config._inlineCallback, this, args, config.tags, config.buttonName);
            };
        }
