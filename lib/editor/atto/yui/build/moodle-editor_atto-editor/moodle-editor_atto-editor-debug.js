...
Y.extend(Editor, Y.Base, {
    ...
    /**
     * A button that was activated after last selection change that is
     * waiting for input.
     *
    ...
    initializer: function() {
        ...
        this._formatQueue = [];
        this.on('atto:selectionchanged', function() {
            this._formatQueue = [];
        });
...
EditorCommand.prototype = {
    ...
    /**
     * The list of functionw with argument to be applied to collapsed selection
     *
     * @property _formatQueue
     * @type Array
     * @private
     */
    _formatQueue: null,
     ...
     */
    applyFormat: function(e, callback, context, args, tags, buttonName) {
console.log(this._formatQueue.length);

        function handleInsert(e, queue, anchorNode, anchorOffset) {
            var selection = window.rangy.getSelection();

            // After something is inputed, select it and apply the formating functions.
            Y.soon(Y.bind(function(e, queue, anchorNode, anchorOffset) {
                var selection = window.rangy.getSelection();
                ...
                queue.forEach(function(op) {
                     op.callback.apply(op.context, [e, op.args]);
                });

                this._formatQueue = [];
                queue = [];

                // Make sure the selection ends on a text node.
                selection = window.rangy.getSelection();
                if (selection.focusNode.nodeName !== '#text') {
                    range = selection.getRangeAt(0);
                    var nodes = range.getNodes();
                    var i = nodes.length;
                    while (i > 0 && nodes[i -1].nodeName !== '#text') {
                        i--;
                    }
                    range.setEndAfter(nodes[i - 1]);
                    selection.setSingleRange(range);
                }

                // Collapse selection to the end of the input.
                ...
                // Selection may not have changed, but the button highting should be updated anyway.
                this._fireSelectionChanged(e);

            }, this, e, queue, anchorNode, anchorOffset));
        }
        ...
        if (this._formatQueue.length > 1) {
            return;
        }

            // Selection is collapsed so listen for input into editor.
            var handle = this.editor.once('input', handleInsert, this, this._formatQueue,
                    selection.anchorNode, selection.anchorOffset);

            // Cancel if selection changes before input.
            this.onceAfter('atto:selectionchanged', function(handle) {
                this._formatQueue = [];
...
EditorSelection.prototype = {
    ...
    setupSelectionWatchers: function() {
        ...
        // Initialize to the expected value after focus.
        this._lastSelection = window.rangy.createRange();
        this._lastSelection.setStart(this.editor.getDOMNode(), 0);
        this._lastSelection.setEnd(this.editor.getDOMNode(), 0);
    ...
    _hasSelectionChanged: function(e) {
        ...
        if (!this._lastSelection || !this._lastSelection.equals(range)) {
            changed = true;
            this._fireSelectionChanged(e);
        }
