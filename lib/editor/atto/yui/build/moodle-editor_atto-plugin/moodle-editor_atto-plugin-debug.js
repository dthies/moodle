...
EditorPluginButtons.prototype = {
     ...
     * @param {string} [config.untag] The tags to unhighlight when this button is activated with uncollapsed selection.
    ...
    addButton: function(config) {
        ...
        if (config.tags) {
            ...
            this._buttonHandlers.push(
                ...
                host.on(['atto:selectionchanged', 'change'], this._handleButtonHighlight,
                   this, config.tags, config.buttonName, tagMatchRequiresAll),
                // Click action may change highlighting.
                this.toolbar.delegate('click', function(e) {
                       Y.soon(Y.bind(this._handleButtonHighlight, this, e, config.tags, config.buttonName, tagMatchRequiresAll));
                     }, 'button', this),
    ...
    _handleButtonHighlight: function(e, tags, buttonName, tagMatchRequiresAll) {
        var host = this.get('host'),
            ...
            // Buttons following tags are toggled on with collapsed selection except with IE.
            toggleHighlighting = tags && selection && selection.collapsed && !Y.UA.ie;

        // If selection changes, no button should show formating change.
        if (e.type === 'atto:selectionchanged') {
            host.activeButton = null;
            host.blockTags = {};
        }

        // If a button is activated with a collapsed selection, toggle the highlighting.
        toggleHighlighting = !!toggleHighlighting &&
                  !!this.buttons[buttonName] &&
                  !!host.activeButton &&
                  this.buttons[buttonName].compareTo(host.activeButton);
        ...
        this._highlightQueue[buttonName] = Y.soon(Y.bind(function() {
            // Check whether a conflicting button overrides this one.
            if (host.blockTags[buttonName]) {
    ...
    addBasicButton: function(config) {
        ...
        config.callback = function() {
            // With IE execCommand does not work right if selection is collapsed.
            var selection = this.get('host').getSelection();
            if (Y.UA.ie && config.tags && selection && selection[0].collapsed) {
                 return;
            }
    ...
    _normalizeCallback: function(config, inheritFrom) {
        ...
        config.callback = Y.rbind(this._callbackWrapper, this, config._callback,
            config.callbackArgs, config.buttonName, config.untag);
     ...
     * @private
     */
    _callbackWrapper: function(e, callback, callbackArgs, buttonName, untag) {
        e.preventDefault();
        ...
        this.get('host').activeButton = creatorButton;

        // Prevent highlight on other buttons if necessary.
        if (untag) {
            this.get('host').blockTags[untag] = true;
            this.get('host').blockTags[buttonName] = false;
        }
