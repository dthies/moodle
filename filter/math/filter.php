...
class filter_math extends moodle_text_filter {
                ...
                while ($endtext <= $match[1] + strlen($match[0])) {
                    ...
                    foreach ($this->delimiters as $d) {
                        if (preg_match('/^' . preg_quote($d[0]) . '(.+?)' . preg_quote($d[1]) . '$/', $container->nodeValue)) {
                            $container->setAttribute('class', 'filter-math-' . $d[2]);
    ...
    private function set_delimiters($text) {
        ...
        if (get_config('filter_math', 'disabledsubplugins')) {
            $disabledsubplugins = explode(',', get_config('filter_math', 'disabledsubplugins'));
        ...
        foreach ($subplugins as $name => $dir) {
            ...
            foreach ($delimiters as $d) {
                if (strrpos($text, $d[1])) {
                    ...
                    $d[] = $name;
                    $this->delimiters[] = $d;
                    $this->subplugins['filter-math-' . $name] = $this->get_plugin($name);
                }
            }
        }
    ...
    public function get_plugin($plugin) {
        return filter_math_plugin::get($plugin);
    }
